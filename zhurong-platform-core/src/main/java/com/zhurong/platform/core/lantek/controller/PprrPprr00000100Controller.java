...
import com.zhurong.platform.core.lantek.api.IPprrPprr00000100Api;
...
public class PprrPprr00000100Controller extends BaseController implements IPprrPprr00000100Api {

    private final PprrPprr00000100Convert convert;
    private final IPprrPprr00000100Service service;

    @Override
    public ApiResponse
            <PageResponse
                    <PprrPprr00000100VO>> page(PprrPprr00000100PageQuery pageQuery) {

        LambdaQueryWrapper<PprrPprr00000100> wrapper =
                Wrappers.lambdaQuery(convert.toEntity(pageQuery));

        wrapper.orderByAsc(PprrPprr00000100::getRecID);

        Page<PprrPprr00000100> page = service.page(
                PageFactory.build(pageQuery),
                wrapper
        );

        List
                <PprrPprr00000100VO> voList = page.getRecords()
                .stream()
                .map(convert::toVO)
                .toList();

        PageResponse
                <PprrPprr00000100VO> response = new PageResponse<>(
                voList,
                page.getTotal(),
                page.getCurrent(),
                page.getSize()
    ...
    @Override
    public ApiResponse
            <PprrPprr00000100VO> getById(Long id) {
        return ApiResponse.success(service.getVOById(id));
    }

    @Override
    public ApiResponse
            <Long> save(@Valid PprrPprr00000100DTO dto) {
    ...
    @Override
    public ApiResponse
            <Boolean> update(Long id, @Valid PprrPprr00000100DTO dto) {
